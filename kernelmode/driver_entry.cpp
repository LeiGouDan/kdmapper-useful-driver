...
NTSTATUS ctl_io(PDEVICE_OBJECT device_obj, PIRP irp) {
	...
	size_t size = 0;
	auto CtrlCode = stack->Parameters.DeviceIoControl.IoControlCode;
	DbgPrint("[drive] Get IO 0x%0lx\n", CtrlCode);
	if (stack) {
		if (buffer && sizeof(*buffer) >= sizeof(info)) {
			if (stack->Parameters.DeviceIoControl.IoControlCode == ctl_hello) {
				DbgPrint("[drive] hello from hidden driver!\n");
			}
			else if (stack->Parameters.DeviceIoControl.IoControlCode == ctl_read) {
...
NTSTATUS driver_initialize(PDRIVER_OBJECT driver_obj, PUNICODE_STRING registery_path) {
	DbgPrint("[drive] 驱动偷偷的加载上了QwQ!\n");

	auto  status = STATUS_SUCCESS;
	UNICODE_STRING  sym_link, dev_name;
	PDEVICE_OBJECT  dev_obj;

	RtlInitUnicodeString(&dev_name, L"\\Device\\u8CI");
	...
	RtlInitUnicodeString(&sym_link, L"\\DosDevices\\u8CI");
...
NTSTATUS DriverEntry(PDRIVER_OBJECT driver_obj, PUNICODE_STRING registery_path) {
	...
	RtlInitUnicodeString(&drv_name, L"\\Driver\\u8CI");
